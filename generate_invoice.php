...
function convertNumberToWords($num) {
    ...
    if ($num < 100) {
        return $words[10 * floor($num / 10)] .
               ' ' . $words[$num % 10];
    }

    if ($num < 1000) {
        return $words[floor($num / 100)] . ' hundred ' 
               . convertNumberToWords($num % 100);
    }

    if ($num < 1000000) {
        return convertNumberToWords(floor($num / 1000)) .
               ' thousand ' . convertNumberToWords($num % 1000);
    }

    return convertNumberToWords(floor($num / 1000000)) .
           ' million ' . convertNumberToWords($num % 1000000);
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Check if 'particularsData' exists before attempting to decode it
    if (isset($_POST['particularsData']) && !empty($_POST['particularsData'])) {
        $particulars = json_decode($_POST['particularsData'], true);
    } else {
        $particulars = []; // Empty array if no data is received
    }
